...
class Wrapper extends React.Component {
  ...
  static defaultProps = {
    label: "Component",
    sizeState: WRAPPER_SIZE_STATE.MINIMIZED,
  };
    ...
    this.setState(() => ({
      position: {
        top: Math.min(
          ...
          Math.max(
            -windowOrigin.top + 5,
            elementOrigin.top + e.clientY - mouseOrigin.top
          )
        ),
        left: Math.min(
          ...
          Math.max(
            -windowOrigin.left + 5,
            elementOrigin.left + e.clientX - mouseOrigin.left
          )
        ),
      },
    }));
  ...
  handleDragStart = (e) => {
    ...
    this.trackedData.elementOrigin = {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <button
            onClick={() => {
              this.props.onSizeChange(-this.props.sizeState);
            }}
...
class TextComponent extends React.Component {
  constructor(props) {
    ...
    this.componentBody =
      this.props.type === TEXT_COMPONENT.PREVIEWER
        ? (style) => (
            <div
        ...
        : (style) => (
            <textarea
              ...
              onInput={(e) => {
                this.props.onInput(
                  DOMPurify.sanitize(marked.parse(e.target.value))
                );
              }}
            >
  ...
  render() {
    if (
      this.currentSize === WRAPPER_SIZE_STATE.MINIMIZED &&
      this.props.sizeState !== WRAPPER_SIZE_STATE.MINIMIZED
    ) {
...
class Background extends React.Component {
  ...
  themeBoth = (colCount, focus) => {
    const words = [
      TEXT_COMPONENT.EDITOR.toUpperCase(),
      TEXT_COMPONENT.PREVIEWER.toUpperCase(),
    ];
  ...
  render() {
    ...
    const backgroundRowSlice =
      this.props.theme.size === APP_SIZE_STATE.BOTH
    ...
    return (
      <div
        ...
        style={{ ...this.BGstyle, ...this.state.size }}
      >
        {" "}
        {backgroundGrid}
      </div>
...
export default class App extends React.Component {
  ...
  handleSizeChange = (type, size) => {
    switch (true) {
      case type == TEXT_COMPONENT.EDITOR &&
        size == WRAPPER_SIZE_STATE.MAXIMIZED:
        this.setState(() => ({ size: APP_SIZE_STATE.EDITOR }));
        break;
      case type == TEXT_COMPONENT.PREVIEWER &&
        size == WRAPPER_SIZE_STATE.MAXIMIZED:
  ...
  render() {
    ...
    switch (this.state.size) {
      case APP_SIZE_STATE.BOTH:
        [editorSize, previewerSize] = [
          WRAPPER_SIZE_STATE.MINIMIZED,
          WRAPPER_SIZE_STATE.MINIMIZED,
        ];
        break;
      case APP_SIZE_STATE.EDITOR:
        [editorSize, previewerSize] = [
          WRAPPER_SIZE_STATE.MAXIMIZED,
          WRAPPER_SIZE_STATE.CLOSED,
        ];
        break;
    ...
    return (
      <React.Fragment>
        <Background
          theme={{ focus: this.state.focus, size: this.state.size }}
        />
        <TextComponent
          type={TEXT_COMPONENT.EDITOR}
